...
function BodyReader(options) {
    ...
    function readBlip(element, blip) {
        ...
        return readImage(findBlipImageFile(blip), altText, getLinkIdBehindPicture(element));
    }

    function getLinkIdBehindPicture (element) {
        var hLinkId = '';

        if (element.first("wp:docPr").children.length) {
            var hLinkElement = element.first("wp:docPr").children.find(function (element) {
                return element.name === 'a:hlinkClick';
            });

            if (hLinkElement) {
                if (hLinkElement.attributes['r:id']) {
                    hLinkId = hLinkElement.attributes['r:id'];
                }
            }
        }
    ...
    function readImageData(element) {
        ...
        if (relationshipId) {
            return readImage(
                ...
                getLinkIdBehindPicture(element));
        } else {
    ...
    function readImage(imageFile, altText, linkIdBehindPicture) {
        var contentType = contentTypes.findContentType(imageFile.path);
        var linkBehindPicture = '';

        if (relationships[linkIdBehindPicture]) {
            linkBehindPicture = relationships[linkIdBehindPicture].target;
        }
